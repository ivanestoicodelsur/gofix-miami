...
async function main() {
  try {
    ...
    if (!spreadsheetId) {
      console.error('ERROR: Provide spreadsheet ID as env GOOGLE_SPREADSHEET_ID or first arg');
      process.exit(2);
    }

    const keyPath = process.env.GOOGLE_SERVICE_ACCOUNT_KEY_PATH || path.join(__dirname, '..', 'service-account.json.json');
    if (!fs.existsSync(keyPath)) {
      console.error('ERROR: service account file missing at', keyPath);
      process.exit(2);
    }
    ...
    console.log('Got token (truncated):', token.slice(0,40) + '...');

    const rangeArg = process.argv[3] || process.env.GOOGLE_SHEETS_RANGE;
    const range = rangeArg || 'Sheet1!A1:E10';
    ...
    console.log('Rows:', JSON.stringify(json.values || [], null, 2));
  } catch (err) {
